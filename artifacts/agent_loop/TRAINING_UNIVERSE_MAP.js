...
const c14 = DATA.c14, p37 = DATA.p37, waves = DATA.waves;
...
el('kpi-cards').innerHTML = [
  ['Pairs C14', c14.n_pairs],
  ['Better rate', fmt(100*c14.n_better/c14.n_pairs,1)+'%'],
  ['CV acc / AUC', fmt(c14.model?.cv_accuracy,2)+' / '+fmt(c14.model?.cv_auc,2)],
  ['Promoted 34p', (c14.promoted||[]).length],
...
new Chart(el('ch-metrics'), { type:'line', data:{ labels:waves.map(w=>w.id), datasets:[
  { label:'CV/LOO acc', data:waves.map(w=>w.acc), borderColor:'#3d9cf0', tension:.25 },
...
const syms = Object.keys(c14.cov_mean).sort((a,b)=> (fuSet.has(a)-fuSet.has(b)) || a.localeCompare(b));
let vals=[]; syms.forEach(s=>tfs.forEach(tf=>{ const v=c14.cov_mean[s]?.[tf]; if(v!=null) vals.push(v); }));
const lo=Math.min(...vals), hi=Math.max(...vals);
let heat = `<table class="heat"><thead><tr><th></th>${tfs.map(t=>`<th>${t}</th>`).join('')}</tr></thead><tbody>`;
syms.forEach(s=>{
  heat += `<tr><td class="f">${s}</td>`;
  tfs.forEach(tf=>{
    const v=c14.cov_mean[s]?.[tf], n=c14.cov_n[s]?.[tf];
...
const tfKeys = Object.keys(c14.tf_overall);
new Chart(el('ch-tf'), { type:'bar', data:{ labels:tfKeys, datasets:[{ label:'mean ΔPnL', data:tfKeys.map(k=>c14.tf_overall[k].mean), backgroundColor:'#3d9cf0aa' }]}, options: chartDefaults });
const symKeys = Object.keys(c14.sym_overall).sort((a,b)=>c14.sym_overall[a].mean-c14.sym_overall[b].mean);
new Chart(el('ch-sym'), { type:'bar', data:{ labels:symKeys, datasets:[{ label:'mean ΔPnL', data:symKeys.map(k=>c14.sym_overall[k].mean), backgroundColor:symKeys.map(k=>fuSet.has(k)?'#e0b45eaa':'#3d9cf0aa') }]}, options:{...chartDefaults, indexAxis:'y'} });

const kindKeys = Object.keys(c14.kind_overall).sort((a,b)=>c14.kind_overall[b].mean-c14.kind_overall[a].mean);
new Chart(el('ch-kind'), { type:'bar', data:{ labels:kindKeys, datasets:[{ label:'mean ΔPnL', data:kindKeys.map(k=>c14.kind_overall[k].mean), backgroundColor:'#5ecf8caa' }]}, options:{...chartDefaults, indexAxis:'y'} });
const eq = c14.kind_by_ac.equity||{}, fu = c14.kind_by_ac.future||{};
...
const br = c14.better_rate_side_kind || {};
const sides = Object.keys(br);
const kinds2 = Array.from(new Set(sides.flatMap(s=>Object.keys(br[s]||{}))));
let hs = `<table class="heat"><thead><tr><th>side \\ kind</th>${kinds2.map(k=>`<th>${k.replace('change_period_','p').replace('add_block_','+')}</th>`).join('')}</tr></thead><tbody>`;
sides.forEach(s=>{
  ...
  kinds2.forEach(k=>{
    ...
    hs += `<td style="background:${heatColor(pct,30,70)}"><div class="cell-v">${pct.toFixed(0)}%</div><div class="cell-n">n=${n}</div></td>`;
  });
  hs += `</tr>`;
});
el('heat-side').innerHTML = hs + `</tbody></table>`;

const sample = (c14.delta_sample||[]).slice().sort((a,b)=>a-b);
...
const loso = c14.loso_1d || {}, losoKeys = Object.keys(loso);
...
const imp = (c14.importance||[]).slice(0,15);
...
el('tbl-top').innerHTML = pairTable(c14.top20||[]);
el('tbl-bot').innerHTML = pairTable(c14.bottom20||[]);
el('tbl-prom').innerHTML = `<thead><tr><th>to</th><th>kind</th><th>mean Δ</th><th>n win</th><th>symbols</th></tr></thead><tbody>`+
  (c14.promoted||[]).map(p=>`<tr><td class="f">${p.to}</td><td>${p.kind}</td><td>${fmt(p.mean_delta,1)}</td><td>${p.n_symbols_win}</td><td class="f">${(p.symbols_win||[]).join(', ')}</td></tr>`).join('')+`</tbody>`;
...
const svg = el('dag');
const nodes = [
  {id:'C1-6', x:60, y:140, label:['Lab C1–C6','trades/interv.']},
  {id:'C7', x:200, y:140, label:['C7 policy','LightGBM']},
  {id:'C9-12', x:360, y:80, label:['C9–C12','multi-symbol']},
  {id:'C13', x:520, y:80, label:['C13','all TF']},
  {id:'C14', x:680, y:80, label:['C14','+futures']},
  {id:'P37', x:680, y:200, label:['P3.7','div calendar']},
  {id:'C15', x:840, y:140, label:['C15 next','SL/TP kinds']},
];
[['C1-6','C7'],['C7','C9-12'],['C9-12','C13'],['C13','C14'],['C14','P37'],['C14','C15'],['P37','C15']].forEach(([a,b])=>{
...
nodes.forEach(n=>{
  ...
  r.setAttribute('rx', 10); r.setAttribute('fill', '#1a2330'); r.setAttribute('stroke', '#3d9cf0');
  g.appendChild(r);
