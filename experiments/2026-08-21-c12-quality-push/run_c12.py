#!/usr/bin/env python3
"""C12 / P3.6: quality push toward a stronger intervention policy.
...
C11 = ROOT / "artifacts/agent_loop/sessions/2026-08-21-c11-multi-symbol-9"
SESSION = ROOT / "artifacts/agent_loop/sessions/2026-08-21-c12-quality-push"
...
def main() -> None:
    # copy prior variants (non-promoted) from C11
    n = 0
    for p in (C11 / "scripts").glob("*.italgo"):
        if p.name.startswith(("27p-", "28p-", "29p-", "30p-", "31p-", "32p-")):
            continue
    ...
    bases = sorted(
        ...
        if not p.name.startswith(("27p-", "28p-", "29p-", "30p-", "31p-", "32p-"))
    )
    added = 0
    for src in bases:
        name = src.name
    ...
    for sym in SYMBOLS:
        for tf in TFS:
            ...
            if not csv.exists():
                ...
                continue
            bars = load_csv(csv)
            # cap 1h to last ~2y trading hours for speed (~12000 bars max)
            if tf == "1h" and len(bars) > 12000:
                bars = bars[-12000:]
    ...
    for (base, kind), st in key_stats.items():
        ...
        if len(sym_wins) >= 5 and st["sum_delta"] / max(1, st["n"]) >= 15:
            ...
            if src.exists():
                ...
                out_name = f"32p-{stem}__{KIND_TAG[kind]}.italgo"
                doc = json.loads(src.read_text())
                doc["meta"]["name"] = f"[cross-sym] {doc['meta'].get('name')}"
                doc["meta"]["tags"] = list(dict.fromkeys((doc["meta"].get("tags") or []) + ["promoted", "c12", "cross-symbol"]))
    ...
    summary = {
        "session": "2026-08-21-c12-quality-push",
        "symbols": SYMBOLS, "timeframes": TFS,
    ...
    (SESSION / "REPORT.md").write_text("\n".join([
        "# C12 quality push (P3.6)",
        "",
        f"- Symbols: {', '.join(SYMBOLS)}",
        f"- TFs: {', '.join(TFS)}",
        f"- Pairs: {len(pairs)} · better: {int(y.sum())} (label: Δ>0 & DD≤1.5×base)",
        f"- CV acc={acc:.3f} AUC={auc}",
        f"- Promoted 32p-*: {len(promoted)} (≥5/10 on 1d)",
        "",
        "## Kind stats",
        json.dumps(kind_stats, indent=2, ensure_ascii=False),
    ]) + "\n")
    (SESSION / "notes.md").write_text(
        "C12/P3.6 quality: +MTSS; RSI50+period×2; 1d+1h; weighted LGBM; DD-aware labels; 32p-*; §7G deploy still backlog.\n"
    )
