#!/usr/bin/env python3
"""C14 / §7F+§7H: +futures (no div-gap) × ALL TFs; +period×0.5; asset_class feature.
...
C13 = ROOT / "artifacts/agent_loop/sessions/2026-08-21-c13-all-tf"
SESSION = ROOT / "artifacts/agent_loop/sessions/2026-08-21-c14-futures-expand"
...
FUTURES = ["CNYRUBF", "GLDRUBF", "IMOEXF"]  # §7H control: no dividend gap
...
def save_doc(doc: dict, path: Path, *, kind: str, base: str, extra: dict) -> None:
    ...
    tags = list(doc["meta"].get("tags") or []) + ["c14", f"intervention:{kind}"]
...
def main() -> None:
    # copy prior variants (non-promoted) from C13
    n = 0
    for p in (C13 / "scripts").glob("*.italgo"):
        if p.name.startswith(("27p-", "28p-", "29p-", "30p-", "31p-", "32p-", "33p-", "34p-")):
            continue
    ...
    bases = sorted(
        ...
        if not p.name.startswith(("27p-", "28p-", "29p-", "30p-", "31p-", "32p-", "33p-", "34p-"))
    )
    added = 0
    for src in bases:
        name = src.name
        ...
        if not (SESSION / "scripts" / f"{stem}__period05x.italgo").exists():
            d = copy.deepcopy(base)
            scale_periods(d, 0.5)
            d["meta"]["name"] = f"{base['meta'].get('name', '')} period×0.5"
            save_doc(d, SESSION / "scripts" / f"{stem}__period05x.italgo",
                     kind="change_period_05x", base=name, extra={"factor": 0.5})
            added += 1
    ...
    KIND_TAG = {
        ...
        "add_block_ema": "ema50", "add_block_adx": "adx25", "add_block_sma200": "sma200", "add_block_rsi50": "rsi50", "change_period_05x": "period05x",
    }
    stable, promoted = [], []
    LIB.mkdir(parents=True, exist_ok=True)
    for (base, kind), st in key_stats.items():
        sym_wins = [s for s, v in st["by_sym"].items() if v["better"] and v["variant_pnl"] > 0]
        if len(sym_wins) >= 6 and st["sum_delta"] / max(1, st["n"]) >= 15:
            item = {
            ...
            if src.exists():
                ...
                out_name = f"34p-{stem}__{KIND_TAG[kind]}.italgo"
                doc = json.loads(src.read_text())
                doc["meta"]["name"] = f"[cross-sym] {doc['meta'].get('name')}"
                doc["meta"]["tags"] = list(dict.fromkeys((doc["meta"].get("tags") or []) + ["promoted", "c14", "cross-symbol"]))
    ...
    summary = {
        "session": "2026-08-21-c14-futures-expand",
        "symbols": SYMBOLS, "timeframes": TFS,
    ...
    (SESSION / "REPORT.md").write_text("\n".join([
        "# C14 futures + all-TF (§7F/§7H)",
        "",
        f"- Symbols: {', '.join(SYMBOLS)}",
        f"- TFs: {', '.join(TFS)}",
        f"- Pairs: {len(pairs)} · better: {int(y.sum())} (label: Δ>0 & DD≤1.5×base)",
        f"- CV acc={acc:.3f} AUC={auc}",
        f"- Promoted 34p-*: {len(promoted)} (≥6 symbols win on 1d; equities+futures)",
    ...
    (SESSION / "notes.md").write_text(
        "C14: +CNYRUBF/GLDRUBF/IMOEXF (futures, no div-gap §7H) × all TF; period×0.5; 34p-*; equities still provisional on div.\n"
    )
