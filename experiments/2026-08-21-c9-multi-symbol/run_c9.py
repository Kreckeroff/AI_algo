#!/usr/bin/env python3
"""C9 / §7F: multi-symbol intervention policy (SBER + GAZP + LKOH).
...
C8 = ROOT / "artifacts/agent_loop/sessions/2026-08-21-c8-p3-expand"
SESSION = ROOT / "artifacts/agent_loop/sessions/2026-08-21-c9-multi-symbol"
...
def main() -> None:
    ...
    for p in src_dir.glob("*.italgo"):
        if p.name.startswith("28p-") or p.name.startswith("27p-"):
            continue
    ...
    for (base, kind), st in key_stats.items():
        ...
        if len(sym_wins) >= 2 and st["sum_delta"] / max(1, st["n"]) >= 20:
            stable.append(
                {
                    "base": base,
                    "kind": kind,
    ...
    for s in stable:
        if s["n_symbols_win"] < 2:
            continue
        src = SESSION / "scripts" / s["variant"]
        if not src.exists():
            continue
        stem = s["base"].replace(".italgo", "")
        out_name = f"29p-{stem}__{KIND_TAG[s['kind']]}.italgo"
        doc = json.loads(src.read_text())
        doc["meta"]["name"] = f"[cross-sym] {doc['meta'].get('name')}"
        doc["meta"]["tags"] = list(
            dict.fromkeys((doc["meta"].get("tags") or []) + ["promoted", "c9", "cross-symbol"])
        )
    ...
    summary = {
        "session": "2026-08-21-c9-multi-symbol",
        "symbols": SYMBOLS,
        "n_pairs": len(pairs),
...
<h1>C9 — мульти-инструмент (§7F): SBER + GAZP + LKOH</h1>
...
"""
    ...
    (SESSION / "notes.md").write_text(
        "C9/§7F: multi-symbol intervention train SBER+GAZP+LKOH; LOSO; cross-symbol 29p-* promote; "
