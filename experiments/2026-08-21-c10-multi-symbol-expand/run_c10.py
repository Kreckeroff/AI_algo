#!/usr/bin/env python3
"""C10 / §7F: expand multi-symbol to 6 tickers (SBER+GAZP+LKOH+ROSN+GMKN+NVTK).
...
SRC_SCRIPTS = ROOT / "artifacts/agent_loop/sessions/2026-08-21-c9-multi-symbol"
SESSION = ROOT / "artifacts/agent_loop/sessions/2026-08-21-c10-multi-symbol-expand"
...
def main() -> None:
    ...
    for p in src_dir.glob("*.italgo"):
        if p.name.startswith(("27p-", "28p-", "29p-", "30p-")):
            continue
    ...
    for (base, kind), st in key_stats.items():
        ...
        if len(sym_wins) >= 3 and st["sum_delta"] / max(1, st["n"]) >= 20:
            stable.append(
                {
                    "base": base,
                    "kind": kind,
    ...
    for s in stable:
        if s["n_symbols_win"] < 3:
            continue
        src = SESSION / "scripts" / s["variant"]
        if not src.exists():
            continue
        stem = s["base"].replace(".italgo", "")
        out_name = f"30p-{stem}__{KIND_TAG[s['kind']]}.italgo"
        doc = json.loads(src.read_text())
        doc["meta"]["name"] = f"[cross-sym] {doc['meta'].get('name')}"
        doc["meta"]["tags"] = list(
            dict.fromkeys((doc["meta"].get("tags") or []) + ["promoted", "c10", "cross-symbol"])
        )
    ...
    summary = {
        "session": "2026-08-21-c10-multi-symbol-expand",
        "symbols": SYMBOLS,
        "n_pairs": len(pairs),
...
<h1>C10 — 6 тикеров (§7F): SBER + GAZP + LKOH</h1>
...
"""
    ...
    (SESSION / "notes.md").write_text(
        "C10/§7F: multi-symbol intervention train SBER+GAZP+LKOH; LOSO; cross-symbol 29p-* promote; "
